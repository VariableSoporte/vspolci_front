...
import {
	ReporteEntregasReporte
} from "../../../../lib/reporte/dominio";
...
export type ReporteEntregasProps = {
  titulo: string;
};
...
export const ReporteEntregas: React.FC<ReporteEntregasProps> = ({ titulo }) => {
  ...
  const [datos, setDatos] = useState<ReporteEntregasReporte[]>(); //tiene todos los datos del response
  ...
  const fetchReportesEntregas = async () => {
    try {
      const respuesta = await serviciosReporte.reporteReporteEntregas(
        userState.id_bodega
      );
  ...
  const filtarDatosFecha = (
    datos_filtrar: ReporteEntregasReporte[],
  ...
  const returnListado = (
    listado_registros: ReporteEntregasReporte[]
  ): dataSolicitudes[] => {
    let listado: dataSolicitudes[] = [];
    listado_registros.map((d) => {
      if (
        !listado.some(
          (p) => p.id_kardex == d.id_kardex && p.fecha_salida == d.fecha_salida
        )
      ) {
        listado.push({
          id_kardex: d.id_kardex,
        ...
        listado = listado.map((l) => {
          if (l.id_kardex == d.id_kardex && l.fecha_salida == d.fecha_salida) {
            return {
              ...l,
  ...
  function agruparPorFechaEInsumo(data: dataSolicitudes[]): AgrupadoPorFecha[] {
    const fechasUnicas = Array.from(
      new Set(data.map((e) => e.fecha_salida))
    ).sort();
    const insumosUnicos = Array.from(new Set(data.map((e) => e.insumo)));
    ...
    data.forEach((registro) => {
      const linea = resultado.find((r) => r.fecha === registro.fecha_salida);
      if (linea) {
        linea[registro.insumo] =
          ((linea[registro.insumo] as number) || 0) + registro.cantidad;
      }
    });
  ...
  function obtenerInsumosUnicos(listado: dataSolicitudes[]): string[] {
    const nombresSet = new Set<string>();
    listado.forEach((item) => nombresSet.add(item.insumo));
  ...
  const exportarPDF = async () => {
    ...
    const datosFiltrados = listaData.filter(
      (item) =>
        insumosSeleccionados.length === 0 ||
        insumosSeleccionados.includes(item.insumo)
    );

    const body = datosFiltrados.map((item) => [
      item.id_kardex.toString(),
      item.insumo,
    ...
    <Box>
      <Box sx={{ display: "flex", gap: 2, mb: 2 }}>
        <FormControl sx={{ m: 1, width: 300 }}>
          ...
          <Select
            ...
            input={<OutlinedInput id="select-insumos-input" label="Insumos" />}
            ...
            MenuProps={MenuProps}
          >
            {obtenerInsumosUnicos(listaData).map((insumo) => (
              <MenuItem
                key={insumo}
                value={insumo}
                style={getStyles(insumo, insumosSeleccionados, theme)}
              >
                {insumo}
              </MenuItem>
            ))}
      ...
      <Box id="reporte-grafico">
        ...
        <ResponsiveContainer width="100%" height={300}>
          <LineChart data={agruparPorFechaEInsumo(listaData)}>
            ...
            {obtenerInsumosUnicos(listaData).map((insumo) => {
              const mostrar =
                insumosSeleccionados.length === 0 ||
                insumosSeleccionados.includes(insumo);
              return (
                mostrar && (
                  <Line
                    key={insumo}
      ...
      <Table>
        ...
        <TableBody>
          {listaData.map((item, i) => {
            ...
            return (
              <TableRow key={i}>
                <TableCell>{item.id_kardex}</TableCell>
